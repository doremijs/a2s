...
export function initFilePath(outputPath: string) {
  ...
  serviceFilePath = resolve(serviceFolder, 'dc.services.ts')
  serviceKeysDescriptionFilePath = resolve(serviceFolder, 'dc.service.keys.d.ts')
  apiRequestDescriptionFilePath = resolve(serviceFolder, 'dc.request.d.ts')
  apisFilePath = resolve(serviceFolder, 'dc.apis.ts')
}
